...
import telran.figures.exceptions.NoCanvasException;
...
public class Page implements Iterable<Shape> {
	...
	public Shape removeShape(long id) {
		int index = Arrays.indexOf(shapes, id);
		if (index == -1) {
			throw new ShapeNotFoundException(id);
		}
	...
	public Shape removeShape(Long[] canvasIds, long id) {
		Canvas canvas = getCanvas(canvasIds);
		int index = Arrays.indexOf(canvas.shapes, id);
		if (index == -1) {
			throw new ShapeNotFoundException(id);
		}
		Shape removedShape = canvas.shapes[index];
		canvas.shapes = Arrays.removeIf(canvas.shapes, shape -> shape.getId() == id);
		return removedShape;
	}
